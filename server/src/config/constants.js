/**
 * Every enum the application uses lives here so that the models, the validators
 * and the seed data can never drift apart from one another.
 */
...
/** Higher number == more authority. Used by `restrictTo` and admin guards. */
const ROLE_RANK = {
  [ROLES.USER]: 1,
  [ROLES.MODERATOR]: 2,
  [ROLES.ADMIN]: 3,
...
/** Human labels, shared with the client through /api/meta. */
...
/** FR-9 shows "current verification status" on the detail screen. */
 ...
 *   awaiting   the timer ran out and we have asked "are you safe?"
 *   safe       they answered
 *   cancelled  they called it off before it was due
 *   escalated  nobody answered inside the grace period, so the SOS went out
 */
...
/** NFR-12: outbound mail lifecycle. */
const MAIL_STATUS = {
  QUEUED: 'queued',
  SENDING: 'sending',
  SENT: 'sent',
...
/** Shared limits. Keeping them here stops magic numbers spreading. */
const LIMITS = {
  ...
  /*
   * Check-in bounds. The floor of one minute is what makes the feature
   * testable and lets someone set a timer for a short walk; the twelve hour
   * ceiling stops a forgotten timer firing in the middle of the night.
   */
  CHECKIN_MIN_MINUTES: 1,
  CHECKIN_MAX_MINUTES: 720,
  CHECKIN_DEFAULT_MINUTES: 30,
  /* How long after the timer they still have to answer before the SOS fires. */

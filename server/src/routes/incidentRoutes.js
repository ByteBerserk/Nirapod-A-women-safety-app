...
/**
 * FR-6 to FR-11. Reading is open to signed-out visitors so the community map
 * is useful before anyone commits to an account; writing always needs a session.
 */
...
router.post(
  ...
  protect,
  limiter.write,
  // Multer runs first: the body is multipart, so req.body does not exist
  // until it has parsed the request.
  uploadIncidentMedia,
  validate({
...
/* FR-25: verification, straight from the report screen. */
router.patch(
  '/:id/status',
  protect,

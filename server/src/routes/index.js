...
// Feature routers. Mounted below in the order they appear.
...
/**
 * NFR-2. A load balancer or uptime monitor hits this. It reports degraded
 * rather than failing outright when the database is down, so the difference
 * between "the process is dead" and "Mongo is having a moment" is visible.
 */
...
/**
 * Every enum the forms need, in one request. Without this the client ends up
 * with its own copy of the category list, and the two drift apart.
 */
router.get('/meta', (req, res) =>
  ok(res, {
    /*
     * Tells the client whether a socket is worth opening, so it never wastes
     * reconnect attempts on a gateway that is not running. The client polls

...
const memberSchema = new mongoose.Schema(
  {
    ...
    /** FR-16: per-group location sharing, opt-in and revocable at any time. */
    ...
    /** Lets a member silence chat without leaving the group. */
...
const inviteSchema = new mongoose.Schema(
  {
    ...
    /** Only the hash is stored - the plain code goes out in the invitation email. */
...
const safetyGroupSchema = new mongoose.Schema(
  {
    ...
    /** FR-17: every member is alerted when one of them raises an SOS. */
...
/** Returns the membership sub-document for a user, or undefined. */
...
/** Owner or group-admin. Used for invite/remove/rename permissions. */
...
/**
 * Creates an invitation and returns the plain code for the email. Any earlier
 * pending invite for the same address is revoked so only one code works.
 */

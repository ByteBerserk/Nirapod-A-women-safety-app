...
/** FR-6: photo, video or audio evidence attached to a report. */
...
/** FR-11: one reaction per person per report. */
...
const incidentSchema = new mongoose.Schema(
  {
    ...
    /** Free-text address, used by the keyword search in FR-9. */
    address: { type: String, trim: true, maxlength: 300, default: '' },
    area: { type: String, trim: true, maxlength: 120, default: '', index: true },
    city: { type: String, trim: true, maxlength: 120, default: '' },

    /** When the incident happened, which is not when it was reported. */
    occurredAt: {
      type: Date,
      required: true,
      validate: {
        validator(value) {
          // Half an hour of slack for a phone with a fast clock.
    ...
    /** FR-6 lets people warn the community without exposing themselves. */
    ...
    /** Denormalised tallies so the feed does not have to unwind `reactions`. */
    reactionCounts: {
    ...
    /** How many people flagged it. Drives the moderation queue ordering. */
...
// Free-text search across the fields FR-9 mentions (keywords, address, area).
...
/** Recomputes the tallies from the source of truth. */
...
/** Reports the public can see. Removed and rejected items disappear. */

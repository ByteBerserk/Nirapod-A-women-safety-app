...
/** Which reaction (if any) the requesting user has left. */
...
/** Feed card. Description is truncated so a list request stays small. */
...
function detail(incident, viewer) {
  ...
  // The flag count is operational data - only staff and the author see it.
  if (isStaff(viewer) || mine) {
    result.reportCount = incident.reportCount || 0;
  }
  // Staff see the true author even on anonymous posts, so abuse can be traced.
...
/**
 * The stripped-down shape the map uses. Sending the full record for a thousand
 * pins would be an easy way to make the map screen crawl (NFR-1).
 */

...
/** Loads a group and asserts the caller is in it. Used by nearly every handler. */
...
export const listGroups = asyncHandler(async (req, res) => {
  ...
  // Pending invitations addressed to this person, so the screen can show a
...
export const inviteMember = asyncHandler(async (req, res) => {
  ...
  // Invite by email address or by username - both end up as an email address.
...
/** Shown before accepting, so the invitee knows what they are joining. */
...
export const leaveGroup = asyncHandler(async (req, res) => {
  ...
  if (isOwner) {
    ...
    if (others.length === 0) {
      // The last person out archives the group rather than orphaning it.
    ...
    // Ownership passes to the longest-serving admin, or the longest-serving
    // member if there is no admin. Leaving a group headless would make it
    // impossible to invite or remove anyone ever again.
    const successor =
...
export const setMemberRole = asyncHandler(async (req, res) => {
  ...
  if (role === GROUP_ROLES.OWNER) {
    // Transferring ownership: the previous owner steps down to admin so the
    // group never ends up with two owners.
...
export const listMessages = asyncHandler(async (req, res) => {
  ...
  // Fetched newest-first for the index, returned oldest-first for the UI.
...
export const sendMessage = asyncHandler(async (req, res) => {
  ...
  // $inc rather than read-modify-write: two people sending at once must not
  // lose a message from the count.
  ...
  // Notify members who muted nothing and are not the sender.
...
export const shareLocation = asyncHandler(async (req, res) => {
  ...
  // A one-off share also drops a pin in the chat, so it is still findable
  // after the live position stops updating.
...
/** Turns off live sharing and wipes the stored position (NFR-5). */
...
/** The live positions of everyone in the group who is currently sharing. */

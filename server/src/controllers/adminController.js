...
/**
 * Everything the dashboard needs in one round trip. Each aggregation is
 * index-backed, and they run concurrently rather than one after another.
 */
export const getDashboard = asyncHandler(async (req, res) => {
  ...
  // Percentage change against the equivalent preceding window.
...
/** Incident counts per category over the window - the pie chart. */
...
/** Reports and SOS activations per day - the trend line. */
export const getTrends = asyncHandler(async (req, res) => {
  ...
  // Days with no activity are missing from the aggregation. Filling them in
  // here stops the chart drawing a straight line across a quiet week.
...
/**
 * FR-24: high-risk locations. Incidents are clustered by rounding coordinates
 * to roughly 1 km, which is accurate enough to point at a neighbourhood
 * without pretending to a precision the reports do not have.
 */
  ...
  const rows = await Incident.aggregate([
    ...
    },
    {
      $project: {
        ...
        category: 1,
        severity: 1,
        // Two decimal places of a degree is about 1.1 km of latitude.
    ...
    { $match: { count: { $gte: 2 } } }, // a single report is not a hotspot
    { $sort: { count: -1 } },
    { $limit: limit },
  ]);
...
export const setUserRole = asyncHandler(async (req, res) => {
  ...
  user.role = role;
  // The role is baked into the access token, so old tokens must stop working.
  user.tokenVersion += 1;
...
export const setUserStatus = asyncHandler(async (req, res) => {
  ...
  if (status === ACCOUNT_STATUS.SUSPENDED) {
    ...
    user.tokenVersion += 1; // ends every open session at once
  } else {
...
/** NFR-12 visibility: what the mail queue is doing right now. */
...
/** Puts every abandoned message back on the queue. */

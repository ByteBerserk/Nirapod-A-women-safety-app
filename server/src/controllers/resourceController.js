...
export const listResources = asyncHandler(async (req, res) => {
  ...
  const [resources, total] = await Promise.all([
    Resource.find(filter)
      .select('-content') // list view never needs the article body
  ...
  // Mark the ones already saved, so the list can show a filled bookmark icon
...
export const getResource = asyncHandler(async (req, res) => {
  // Accepts either the slug or the id, so links stay readable.
...
export const addBookmark = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Saving something twice is not an error worth showing the user.
    if (error.code === 11000) {
...
export const listBookmarks = asyncHandler(async (req, res) => {
  ...
  // Resolve the saved items in two queries rather than N, and quietly skip
...
export const removeBookmark = asyncHandler(async (req, res) => {
  // Deleting by target rather than by bookmark id means the UI can toggle a
  // star without first looking up which bookmark row it belongs to.

...
/** FR-12 (reporting content) and FR-13 (the moderator queue). */
...
describe('Moderation', () => {
  ...
  describe('Resolving a report (FR-13)', () => {
    /** Sets up an incident that has been reported once, plus a moderator. */
    ...
    it('restores content that was removed by mistake', async () => {
      ...
      // A second report is needed because a resolved one cannot be reused.
  ...
  /*
   * FR-13 restore. This whole path used to be unreachable: removing content
   * leaves its report "actioned", and the guard at the top of resolveReport
   * rejected every report that was not still open - including the one you would
   * need to reopen to undo the removal. A moderator who removed the wrong post
   * had no way back, while the UI cheerfully offered the button.
   */

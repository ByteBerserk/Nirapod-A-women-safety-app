...
/** Pure logic, tested without touching the database or the HTTP layer. */

describe('geo helpers', () => {
  it('measures a known distance', () => {
    // Dhaka to Chittagong is about 215 km.
  ...
  it('handles the antimeridian without going the long way round', () => {
    ...
    // About 22 km, not most of the way around the planet.
  ...
  it('accepts the several spellings of longitude', () => {
    ...
    // Strings from a query string still parse.
...
describe('query helpers', () => {
  ...
  it('escapes regex metacharacters in a search term', () => {
    ...
    // Constructing the RegExp is the real assertion - an unescaped "+" throws.
...
describe('Client URL resolution', () => {
  /*
   * Not a loud failure if it goes wrong: the app runs perfectly and every
   * password reset and SOS tracking link quietly points somewhere useless.
   *
   * The LAN address is injected rather than detected, so these assertions do
   * not depend on the network the suite happens to run on.
   */
  ...
  /*
   * The important one. Emails leave this machine, so "localhost" in a tracking
   * link resolves to the recipient's own phone and shows a connection error
   * instead of a map - the single worst way for this application to fail.
   */
  ...
  /*
   * The failure this guards against: a tunnel that was killed rather than
   * stopped politely used to leave its address behind in configuration, and the
   * app went on emailing links to a hostname that no longer resolved. Nothing
   * looked wrong until a contact could not open the map.
   */

...
/** FR-24 (analytics dashboards) and FR-25 (user and content administration). */
...
describe('Administration', () => {
  ...
  describe('Analytics (FR-24)', () => {
    it('breaks incidents down by category with percentages', async () => {
      ...
      // Sorted by count, so the commonest category leads.
    ...
    it('treats a repeatedly reported spot as a hotspot', async () => {
      ...
      // Three reports within a few metres fall in the same grid cell.
      await createIncident(member.token);
  ...
  describe('User management (FR-25)', () => {
    ...
    /*
     * The guard counts *active* administrators, so the case it really protects
     * against is a second admin existing but being suspended: demoting them
     * would leave nobody who can actually sign in and administer the platform.
     * The caller can never be the last admin in this check, because changing
     * your own role is refused a few lines earlier.
     */

...
/** FR-6 and FR-7: submit a report, categorise it, attach evidence. */
...
export default function ReportIncident() {
  ...
  /* Start from where the reporter is, then let them drag the pin. */
  useEffect(() => {
  ...
  // Object URLs for previews must be revoked, or the blobs leak until reload.
  useEffect(() => {
  ...
  const pickLocation = async (point) => {
    ...
    try {
      ...
      // A pin without a resolved street name is still a usable report.
    }
  };
  ...
  const submit = async (event) => {
    ...
    try {
      // multipart, because of the attachments.

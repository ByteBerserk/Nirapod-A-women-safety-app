...
/**
 * FR-2 and FR-3, from the other side.
 *
 * An emergency contact opens this from the email they were sent. They are not
 * signed in and may never have heard of this app, so the page has to work with
 * no account, on any device, and put the two useful things - where she is, and
 * how to help - above everything else.
 *
 * It runs its own socket connection rather than using SocketContext, because
 * that context is for authenticated users and this visitor has no token.
 */
export default function TrackSos() {
  ...
  /*
   * A fallback poll. Sockets are blocked on some corporate and hotel networks,
   * and a contact on such a network still needs the position to move.
   */
  useEffect(() => {

 ...
 *
 * Set a timer before doing something you are unsure about. When it runs out the
 * app asks whether you are safe; if you do not answer within the grace period
 * your emergency contacts get the same alert the SOS button sends.
 */
...
/** Seconds as m:ss, or h:mm:ss once it is over an hour. */
...
export default function CheckIn() {
  ...
  /*
   * One second locally for the display, and a refetch every fifteen so the
   * server stays the authority on when this escalates - a clock that has
   * drifted must never be the reason someone thinks they have longer.
   */
  useEffect(() => {
  ...
  /** Seconds remaining, counted down locally from the last server figure. */
  ...
  const start = async (event) => {
    ...
    try {
      // Best effort. A check-in with no location still works; the alert simply
      // tells the contact to call instead.
      let point = null;
      try {

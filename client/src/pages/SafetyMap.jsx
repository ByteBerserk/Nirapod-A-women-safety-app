...
export default function SafetyMap() {
  ...
  // Panning fires a lot of bounds events; a debounce turns a drag into one
  ...
  useEffect(() => {
    getPosition({ enableHighAccuracy: false, maximumAge: 300000 })
      ...
      .catch(() => {
        // Location denied: start on a sensible default rather than a blank map.
  ...
  // First load, once we know where to look.
  useEffect(() => {
    if (centre && !view) loadPins({ centre, radius: 5000 });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [centre]);

  // Reload when the filters change, keeping the current viewport.
  useEffect(() => {

...
/** FR-21: safety tips, self-defence, legal rights, helplines. */
...
export default function Resources() {
  ...
  const toggleBookmark = async (resource, event) => {
    // The card is a link; the star inside it must not navigate.
        ...
        <>
          <div className="grid grid-3">
            {resources.map((resource) => {
              ...
              return (
                /*
                 * An <article> with a stretched link, rather than a <Link>
                 * wrapped around everything. The helpline numbers below are
                 * `tel:` anchors, and an anchor inside an anchor is invalid
                 * HTML - React warns about it and browsers disagree about
                 * which destination a click should follow.
                 */
                    ...
                    <ul className="helpline-preview">
                      {resource.contactNumbers.slice(0, 3).map((entry) => (
                        <li key={entry.number}>
                          ...
                          {/* Above the stretched link, so tapping a number dials
                              it instead of opening the article. */}

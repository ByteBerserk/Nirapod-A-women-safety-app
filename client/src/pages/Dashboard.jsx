...
/**
 * The home screen. The SOS button is the first and largest thing on it - NFR-6
 * asks for critical features to be reachable in one tap, and everything else
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      // allSettled: the dashboard should still render if one panel's request
      // fails. A broken map lookup must not blank the SOS button.
  ...
  // Location is requested separately and quietly - the dashboard is useful
  // without it, so a denied prompt should not look like an error.
  useEffect(() => {
    let cancelled = false;

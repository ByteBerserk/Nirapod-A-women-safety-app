...
/**
 * Every server call the app makes, grouped by feature. Components import from
 * here rather than calling axios directly, so a URL change touches one file.
 * Each function returns the `data` block of the envelope, not the axios
 * response - callers should not have to know about `.data.data`.
 */

const unwrap = (response) => response.data?.data ?? null;
/** For the handful of places that need the human-readable message too. */
...
/** The four calls below reach OpenStreetMap, so they get the longer allowance. */

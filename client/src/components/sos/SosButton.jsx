...
/**
 * FR-4 and NFR-6: the one-tap emergency button.
 *
 * "One tap" in the requirement means one deliberate action, not one accidental
 * brush. A bare click handler fires in a pocket or a handbag and mails
 * everyone you know. So the button arms on press and fires after a 1.2 second
 * hold, with the progress ring making the wait visible. Releasing early
 * cancels. Keyboard users get Enter/Space with the same hold behaviour.
 */
...
export function SosButton({ size = 'large', showLabel = true }) {
  ...
  const fire = useCallback(async () => {
    stopHold();
    // Haptic confirmation on phones that support it - useful when the screen
  ...
  // A hold interrupted by an unmount would otherwise leave a rAF running.
...
/** Floating variant, pinned above the bottom navigation on every signed-in page. */

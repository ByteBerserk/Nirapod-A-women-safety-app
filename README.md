...
> **Alerting is email-only.** There is no SMS anywhere in this project. Every
> alert path is email plus an in-app notification, delivered over a socket when
> one is available and by polling when it is not.

Runs locally: one Node process for the API and realtime gateway, one Vite dev
server for the client, and MongoDB alongside them.
